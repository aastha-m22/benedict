...
class SlackFormatter:
    ...
    def markdown_to_mrkdwn(text: str) -> str:
        ...
        text_with_placeholders = re.sub(inline_code_pattern, replace_inline_code, text_with_placeholders)
        
        # Convert headings to bold (process in reverse order to avoid double conversion)
        text_with_placeholders = re.sub(r'^### (.+)$', r'*\1*', text_with_placeholders, flags=re.MULTILINE)
        text_with_placeholders = re.sub(r'^## (.+)$', r'*\1*', text_with_placeholders, flags=re.MULTILINE)
        text_with_placeholders = re.sub(r'^# (.+)$', r'*\1*', text_with_placeholders, flags=re.MULTILINE)
        ...
        text_with_placeholders = re.sub(r'(?<!\*)\*([^*]+?)\*(?!\*)', r'_\1_', text_with_placeholders)
        
        # Convert links: [text](url) to <url|text>
        text_with_placeholders = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'<\2|\1>', text_with_placeholders)
        ...
        for i, code_block in enumerate(code_blocks):
            text_with_placeholders = text_with_placeholders.replace(f"__CODE_BLOCK_{i}__", code_block)
...
class BlockKitFormatter:
    ...
    def create_section(text: str, fields: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        block: Dict[str, Any] = {
            ...
            "text": {
                "type": "mrkdwn",
                "text": text[:MAX_TEXT_IN_BLOCK]  # Enforce limit
            }
        }
    ...
    def create_code_block(code: str, language: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "text": {
                "type": "mrkdwn",
                "text": code_text[:MAX_TEXT_IN_BLOCK]
            }
        }
    ...
    def format_message(
        ...
        if not use_block_kit:
            return {
                "text": formatted_text[:MAX_MESSAGE_LENGTH]
            }
    ...
    def format_status_message(
        ...
        for i in range(0, len(field_texts), 2):
            ...
            blocks.append(BlockKitFormatter.create_section(
                text="",  # Empty text, using fields only
                fields=field_pair
            ))
